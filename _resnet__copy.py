...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
                 base_width=64, remove_last_relu=False):
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.bn3 = CBN(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
        self.downsample = downsample
        self.stride = stride
...
class ResNet(nn.Module):
    ...
    def __init__(self, block_type, layers, zero_init_residual=False,
        ...
        self.layer1 = self._make_layer(
            block_type, 64, layers[0], remove_last_relu=False)
        self.layer2 = self._make_layer(
            block_type, 128, layers[1], stride=2, remove_last_relu=False)
        self.layer3 = self._make_layer(
            block_type, 256, layers[2], stride=2, remove_last_relu=False)
        self.layer4 = self._make_layer(
            block_type, 512, layers[3], stride=2, remove_last_relu=remove_last_relu)
        self.remove_last_relu = remove_last_relu
    ...
    def _make_layer(self, block_type, planes, blocks, stride=1, remove_last_relu=False):
        if block_type == 'basic':
            block = BasicBlock
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                # CBN(planes * block.expansion),
            )

        layers = []
        layers.append(block(self.inplanes, planes, stride, downsample, self.groups, 
                            self.base_width, remove_last_relu=False))
